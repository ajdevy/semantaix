...
def test_webhook_accepts_text_message_and_returns_trace():
    ...
    response = client.post("/telegram/webhook", json=load_fixture("update_message_text_basic.json"))
...
def test_webhook_ignores_empty_text_message():
    client = TestClient(bot_app)
    response = client.post("/telegram/webhook", json=load_fixture("update_message_text_empty.json"))
    assert response.status_code == 200
...
def test_webhook_rejects_malformed_payload():
    client = TestClient(bot_app)
    response = client.post("/telegram/webhook", json=load_fixture("update_malformed_missing_core.json"))
...
def test_webhook_ignores_callback_query_update():
    client = TestClient(bot_app)
    response = client.post("/telegram/webhook", json=load_fixture("update_callback_query_valid.json"))
    assert response.status_code == 200
    assert response.json()["status"] == "ignored"
...
def test_webhook_ignores_edited_message_update():
    client = TestClient(bot_app)
    response = client.post("/telegram/webhook", json=load_fixture("update_edited_message_valid.json"))
    assert response.status_code == 200
    assert response.json()["status"] == "ignored"
...
def test_webhook_ignores_non_text_message():
    client = TestClient(bot_app)
    response = client.post("/telegram/webhook", json=load_fixture("update_non_text_message_photo.json"))
    assert response.status_code == 200
    assert response.json()["status"] == "ignored"
