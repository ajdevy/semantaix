...
- **`GET /calendar/projects/{project_id}/settings`** (Epic 11 story 11.08) continues to surface the service rules subset — it now reads via `ProjectServiceRepository.list_calendar_eligible` instead of `CalendarSettingsRepository.list_service_rules` (or via the Epic-11 alias, which delegates per 12.01). No new endpoint here; the read path naturally picks up the canonical store.

### Out of Scope
- The `/service` slash command + `/calendar_service` alias DM hint (story 12.03).
- The NL ops endpoints `POST /api/projects/{id}/services/nl-ops*` (story 12.04).
- The catalog-answerer cutover + prose rendering (story 12.06).
...
- Deprecation log emitted on every alias-path call (log-only at the endpoint surface; user-facing DM hint is owned by 12.03).
