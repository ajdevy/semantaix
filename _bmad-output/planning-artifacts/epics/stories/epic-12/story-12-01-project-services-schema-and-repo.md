# Story 12.01 — `project_services` schema, repository, and migration

## Objective
Lay the data + repository foundation for Epic 12: rename Epic 11's `calendar_service_rules` to `project_services` (in the same `.data/semantaix_calendar.db`), add the four catalog columns, enforce `UNIQUE(project_id, lower(name))`, and ship the canonical `ProjectServiceRepository` (sync `sqlite3` + per-`(project_id, lower(name))` `asyncio.Lock` single-flight upsert). Also create the `services_nl_op_sessions` table in `.data/semantaix_nl_ops.db` (consumed by story 12.04) and the new data file `data/russian_calendar_terms.json` (consumed by story 12.06's prose renderer). This story ships no customer-visible behavior — it is the substrate every later story builds on. Architecture reference: `_bmad-output/planning-artifacts/architecture.md` lines 222–252; FR reference: PRD §FR-23.
...
### Out of Scope
- The `ServicesNlOpsRepository` class + `parse_service_intent` parser (story 12.04 owns the session repository + regex parser; this story only creates the empty table).
- The api endpoints (story 12.02), slash command (12.03), NL dialog (12.05), prose renderer (12.06), catalog-answerer cutover (12.06).
...
- **`secrets` / `hmac` / `Fernet` not used here** — those land in 12.04 (session tokens). This story only does schema + CRUD.
...
- **Russian rendering data file** — `data/russian_calendar_terms.json` is loaded ONCE at process startup (lazy module-level cache) by the renderer in 12.06; this story just creates the file with day-code + month-genitive + closed-prefix entries. No code in this story reads it.
...
### Contract
- (No api endpoints in this story; contract tests land in 12.02.)

### Integration
- `tests/test_api_startup_bootstrap_epic12.py` — boot the api with a fresh `.data/`; assert `semantaix_calendar.db` contains `project_services` with the final schema + `semantaix_nl_ops.db` contains `services_nl_op_sessions`.
...
- `services_nl_op_sessions` table created in `semantaix_nl_ops.db` (consumed by 12.04).
- `data/russian_calendar_terms.json` exists and parses (consumed by 12.06).
