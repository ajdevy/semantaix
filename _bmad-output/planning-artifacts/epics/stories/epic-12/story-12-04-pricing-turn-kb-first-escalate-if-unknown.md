...
- Persona prompt files:
  - `system_prompts/sales_pricing_hit.txt` — one-sentence Russian quote with the verbatim number; persona-name placeholder; price-as-data instruction ("ONLY quote numbers present verbatim in the snippet — never round, infer, or convert currency").
