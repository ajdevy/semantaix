...
Implementation order follows the dependency graph (12.01 is the foundation and blocks every later story; 12.02 / 12.03 / 12.05 are parallel after 12.01; 12.05b chains after 12.05):
...
12.01 sales DB schema + repos (state, services, client_materials, followup_queue)   ← foundation, blocks all
  ...
  ├── 12.03 SalesPersonaAnswerer greeting + intent scoping                           (parallel after 12.01)
  └── 12.05 autonomous client materials dispatch (/material + selector + endpoint)   (parallel after 12.01)
        └── 12.05b KB-upload → automatic client materials analysis                   (after 12.05)
              ...
              12.08 proactive +1d follow-up (scheduler job)                          (after 12.03)
                    12.09 pipeline wiring + escalation seams + e2e signoff           (last; depends on all)
```
